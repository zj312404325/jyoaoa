...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		// 校验用户名密码
		User user = getSystemService().getUserByLoginNameOL(token.getUsername());
		if (user != null) {
		...
		if (!Global.TRUE.equals(Global.getConfig("user.multiAccountLogin"))){
			...
			if (sessions.size() > 0){
				...
				if (UserUtils.getSubject().isAuthenticated()){
					String sessionid= FormatUtil.toString(Global.userSessionMap.get(principal.getId()));
					if(FormatUtil.isNoEmpty(sessionid)){//此用户已登录，踢出
...
//					throw new AuthenticationException("msg:账号已在其它地方登录，请重新登录。");
					throw new AuthenticationException("msg:您的账号登录信息已过期，请重新登录。");
				}
			}
		}
