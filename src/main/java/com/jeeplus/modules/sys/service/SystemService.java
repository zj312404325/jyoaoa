...
public class SystemService extends BaseService implements InitializingBean {
	...
	public User getUserByLoginNameOL(String loginName) {
		return UserUtils.getByLoginNameOL(loginName);
	}
	...
	@SuppressWarnings("unchecked")
	public List<User> findAllUserByOfficeId(String officeId,String udata) {
		//List<User> list = (List<User>)CacheUtils.get(UserUtils.USER_ALL_CACHE, UserUtils.USER_ALL_CACHE_LIST_BY_OFFICE_ID_ + officeId);
		List<User> list =null;
		if (list == null){
			User user = new User();
			if(FormatUtil.isNoEmpty(udata)&&udata.equals("1")){//查询用户时过滤的已提交离职单的用户
				user.setUdata(udata);
			}
			user.setOffice(new Office(officeId));
			list = userDao.findAllUserByOfficeId(user);
			//CacheUtils.put(UserUtils.USER_ALL_CACHE, UserUtils.USER_ALL_CACHE_LIST_BY_OFFICE_ID_ + officeId, list);
		}
		return list;
	}
	
	@SuppressWarnings("unchecked")
	public List<User> findUserByOfficeRole(String officeId,String roleId,String usertype) {
		List<User> list = null;
		User user = new User();
		user.setOffice(new Office(officeId));
		user.setRole(new Role(roleId));
		user.setUserType(usertype);
		list = userDao.findUserByOfficeOrRole(user);
		return list;
	}
	
	@SuppressWarnings("unchecked")
	public List<User> findUserByDeptOfficeRole(String deptId,String officeId,String roleId,String usertype) {
		List<User> list = null;
		User user = new User();
		user.setOfficeTrueId(deptId);
		user.setOffice(new Office(officeId));
		user.setRole(new Role(roleId));
		user.setUserType(usertype);
		list = userDao.findUserByOfficeOrRole(user);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void deleteUser(User user) {
		...
		//删除入职信息sys_user_info
		//userInfoDao.deleteUserInfoByUser(user);
		//删除用户表
		userDao.delete(user);
...
//		// 清除权限缓存
//		systemRealm.clearAllCachedAuthorizationInfo();
		oagroupdtlDao.deleteByLogic(new Oagroupdtl(user));
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public static boolean printKeyLoadMessage(){
		...
		sb.append("\r\n======================================================================\r\n");
		sb.append("\r\n    欢迎使用 "+Global.getConfig("productName")+"  - Powered By http://www.lemote.com\r\n");
		sb.append("\r\n======================================================================\r\n");
	...
	public List<User> findListByCompany(User user) {
		List<User> list = userDao.findListByCompany(user);
		return list;
	}
	...
	public void saveOffice(Office office) {
		// 更新部门与菜单关联
		officeDao.deleteOfficeMenu(office);
		if (office.getMenuList().size() > 0){
			officeDao.insertOfficeMenu(office);
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List<String> getMenuIds(String id) {
		return officeDao.getMenuIds(id);
	}
}
