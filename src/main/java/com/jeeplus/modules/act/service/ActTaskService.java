...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		...
		if (act.getEndDate() != null){
			todoTaskQuery.taskCreatedBefore(FormatUtil.dayEnd(act.getEndDate()));
		}
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
		...
		if (act.getEndDate() != null){
			toClaimQuery.taskCreatedBefore(FormatUtil.dayEnd(act.getEndDate()));
		}
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	...
	public Page<Act> historicList(Page<Act> page, Act act){
		...
		if (act.getEndDate() != null){
			histTaskQuery.taskCompletedBefore(FormatUtil.dayEnd(act.getEndDate()));
		}
...
//	}
	 ...
	 * @return
	 */
	public Page<Map> applyList(Page<Map> page, Act act){
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
		
		Map<String,Object> mapTemp=new HashMap<String, Object>();
		mapTemp.put("startuserid", userId);
		if (StringUtils.isNotBlank(act.getProcDefKey())){
			mapTemp.put("procDefKey", act.getProcDefKey());
		}
		if (act.getBeginDate() != null){
			mapTemp.put("agotime", FormatUtil.DateToString(act.getBeginDate(), "yyyy-MM-dd"));
		}
		if (act.getEndDate() != null){
			mapTemp.put("futuretime", FormatUtil.DateToString(act.getEndDate(), "yyyy-MM-dd")+" 23:59:59");
		}
		Integer count= actDao.findProcessInstCount(mapTemp);
		
		// 查询总数
		page.setCount(count);
		page.initialize();
		
		int currpage=page.getFirstResult()/page.getMaxResults()+1;
		int start=(currpage - 1) * page.getMaxResults() + 1;
		int end=currpage*page.getMaxResults();
		mapTemp.put("pageStart", start);
		mapTemp.put("pageEnd", end);
		List<Map> listMap=actDao.findProcessInstList(mapTemp);
		for (Map map1 : listMap) {
			ProcessDefinition pd=ProcessDefCache.get((String)map1.get("PROCESSDEFINITIONID"));
			act.setProcDef(pd);
			map1.put("STARTTIME",FormatUtil.timestampToString(map1.get("STARTTIME")));
			map1.put("version", pd.getVersion());
			map1.put("flowname", pd.getName());
			page.getList().add(map1);
		}
		return page;
	}
	...
	public void updateTask(Map map){
		actDao.updateHiTask(map);
		actDao.updateRunTask(map);
	}
	
	public String getNextTask(String processInstanceId) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		Execution execution = runtimeService.createExecutionQuery().executionId(processInstanceId).singleResult();//执行实例
		Object property = PropertyUtils.getProperty(execution, "activityId");
		String activityId = "";
		if (property != null) {
		...
		for (ActivityImpl activity : activitiList) {
			...
			if (id.equals(activityId)) {
				//currentActiviti = true;
				System.out.println("当前任务："+activity.getProperty("name")); //输出某个节点的某种属性
				
				TaskDefinition oo=nextTaskDefinition(activity, activity.getId(),"${iscorrect==1}");
				
				List<PvmTransition> outTransitions = activity.getOutgoingTransitions();//获取从某个节点出来的所有线路
				for(PvmTransition tr:outTransitions){
					PvmActivity ac = tr.getDestination(); //获取线路的终点节点
					System.out.println("下一步任务任务："+ac.getProperty("name"));
					System.out.print("下一步任务id:"+ac.getId());
					result=ac.getId();
				}
				break;
			}
