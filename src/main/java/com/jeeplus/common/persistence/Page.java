...
public class Page<T> {
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if (StringUtils.isNumeric(size)){
			String oldPageSize=CookieUtils.getCookie(request, "pageSize");
			if(FormatUtil.isNoEmpty(oldPageSize)&&!oldPageSize.equals(size)){
				this.setPageNo(1);
			}
			...
			if(FormatUtil.isNoEmpty(no)&&!no.equals(size)){
				this.setPageNo(1);
			...
			String pageno = CookieUtils.getCookie(request, "pageNo");
			String pagesize = CookieUtils.getCookie(request, "pageSize");
			this.setPageNo(FormatUtil.toInteger(pageno)==0?1:FormatUtil.toInteger(pageno));
			this.setPageSize(FormatUtil.toInteger(pagesize)==0?10:FormatUtil.toInteger(pagesize));
		}*/
		// 设置排序参数
...
//	}
	...
	public String getAppPageString(int pageSize) {
		StringBuilder sb = new StringBuilder();
		long startIndex = (pageNo-1)*pageSize + 1;
		long endIndex = pageNo*pageSize <=count? pageNo*pageSize:count;

		sb.append("显示第 "+startIndex+" 到第 "+ endIndex +" 条记录，总共 "+count+" 条记录");
		return sb.toString();
	}
}
