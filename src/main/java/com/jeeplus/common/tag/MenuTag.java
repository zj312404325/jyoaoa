...
public class MenuTag extends TagSupport {
	...
	private static String getChildOfTree(Menu parent, int level, List<Menu> menuList) {
		...
		if(FormatUtil.isNoEmpty(parent)){
			String href = "";
			...
			if ((parent.getHref() == null || parent.getHref().trim().equals("")) && parent.getIsShow().equals("1")) {//如果是父节点且显示
				...
				menuString
						.append("<a href=\""
								...
								+ parent.getName()
								+ "</span><span class=\"fa arrow\"></span><span class=\"text-danger\" id=\""+parent.getId()+"\"></span></a>");
				}
				if (level == 1) {
				...
				menuString.append("<a class=\"J_menuItem\"  href=\"" + href
						+ "\" ><i class=\"fa "+parent.getIcon()+"\"></i> <span class=\"nav-label\">"+parent.getName()+"</span><span class=\"text-danger\" id=\""+parent.getId()+"\"></span></a>");
			}
			if (level > 0) {
				menuString.append("</li>");
			}
