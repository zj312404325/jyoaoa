...
//		System.out.println("有新连接加入！当前在线人数为"+j.toString());
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith(Constant._online_user_)){//用户上线
			...
			Constant.User_Status_Map.put(userId,Constant._online_user_);
			
			//通知所有用户更新在线信息
			Collection<String> onlineUsers = ChatServerPool.getOnlineUser();
			AjaxJson j = new AjaxJson();
			j.put("data", onlineUsers);
			j.put("userdata", Constant.User_Status_Map);
			ChatServerPool.sendMessage("_online_all_status_on_"+j.getJsonStr());//通知所有用户更新在线信息
		...
		}if(null != message && message.startsWith(Constant._leave_user_)){//用户隐身
			System.out.println("用户隐身");
			String userId = message.replaceFirst(Constant._leave_user_, "");
			this.userjoin(userId,conn);
			Constant.User_Status_Map.put(userId,Constant._leave_user_);

			Collection<String> onlineUsers = ChatServerPool.getOnlineUser();
			AjaxJson j = new AjaxJson();
			j.put("data", onlineUsers);
			j.put("userdata", Constant.User_Status_Map);
			ChatServerPool.sendMessage("_online_all_status_hide_"+j.getJsonStr());//通知所有用户更新在线信息
...
//			j.put("data", onlineUsers);
//			j.put("userdata", Constant.User_Status_Map);
//			ChatServerPool.sendMessage("_online_all_status_off_"+j.getJsonStr());//通知所有用户更新在线信息
		}if(null != message && message.startsWith(Constant._leaveout_user_)){//用户离开
			System.out.println("用户离开");
			String userId = message.replaceFirst(Constant._leaveout_user_, "");
			this.userjoin(userId,conn);
			Constant.User_Status_Map.put(userId,Constant._leaveout_user_);
			...
			j.put("data", onlineUsers);
			j.put("userdata", Constant.User_Status_Map);
			ChatServerPool.sendMessage("_online_all_status_leave_"+j.getJsonStr());//通知所有用户更新在线信息

		}if(null != message && message.startsWith(Constant._offline_user_)){//用户隐身
			System.out.println("用户离线");
			String userId = message.replaceFirst(Constant._offline_user_, "");
			Constant.User_Status_Map.put(userId,Constant._offline_user_);
			Collection<String> onlineUsers = ChatServerPool.getOnlineUser();
			AjaxJson j = new AjaxJson();
			j.put("data", onlineUsers);
			j.put("userdata", Constant.User_Status_Map);
			...
			if("group".equals(type)){//如果是群聊
				...
				for(LayGroupUser lgUser:layGroupUserlist){
					...
					if(toUserConn != null){
						...
						map.put("cnt3", "-1");
						maptemp.put("url", "");
						maptemp.put("title", "你收到了一条新的即时消息,请及时查看！");
						lst.add(maptemp);
						map.put("msg", JSONArray.fromObject(lst).toString());
						String sendmsg = JSONObject.fromObject(map).toString();

						String sendmessage = Constant._remind_window_+sendmsg;
						ChatServerPool.sendMessageToUser(toUserConn,sendmessage);
					}else{
