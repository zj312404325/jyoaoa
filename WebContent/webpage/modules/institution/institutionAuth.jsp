...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			// 用户-菜单
			var zNodes=[
					<c:forEach items="${institutionList}" var="menu">{id:"${menu.id}", pId:"${not empty menu.parent.id?menu.parent.id:0}", name:"${not empty menu.parent.id?menu.title:'制度列表'}"},
		            </c:forEach>];
			// 初始化树结构
			...
			// 默认选择节点
			var ids = "${role.institutionIds}".split(",");
...
<body>
	<form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/saveIn" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<form:hidden path="institutionIds"/>
	</form:form>
</body>
</html>