...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="performanceInterview" action="${ctx}/checkmodel/performanceInterview/managerlist" method="post" class="form-inline">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<table:sortColumn id="orderBy" name="orderBy" value="${page.orderBy}" callback="sortOrRefresh();"/><!-- 支持排序 -->
		<div class="form-group">
			<span>部门：</span>
			<sys:treeselect id="officeid" name="officeid" value="${performanceInterview.officeid}" labelName="officename" labelValue="${performanceInterview.officename}"
							title="部门" url="/sys/office/treeData?type=2" cssClass=" form-control input-sm" allowClear="true" notAllowSelectParent="false"/>
			<span>员工编号：</span>
			<form:input path="userno" htmlEscape="false" maxlength="10" value="${performanceInterview.userno}" class=" form-control input-sm"/>
			<span>员工姓名：</span>
			<form:input path="username" htmlEscape="false" maxlength="10" value="${performanceInterview.username}" class=" form-control input-sm"/>
			<span>开始时间：</span>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<c:forEach items="${page.list}" var="performanceInterview">
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${(performanceInterview.overachievement!=null&&erformanceInterview.overachievement!='')||(performanceInterview.interviewcomment!=null&&erformanceInterview.interviewcomment!='')||(performanceInterview.development!=null&&erformanceInterview.development!='')}">
							已评
						</c:when>
						<c:otherwise>
							未评
				...
				<td>
					<shiro:hasPermission name="checkmodel:performanceInterview:view">
						<a href="#" onclick="openDialogView('查看绩效考核面谈表(${performanceInterview.username})', '${ctx}/checkmodel/performanceInterview/form?id=${performanceInterview.id}&type=1','800px', '500px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i> 查看</a>
					</shiro:hasPermission>
					<%-- <shiro:hasPermission name="checkmodel:performanceInterview:edit">
