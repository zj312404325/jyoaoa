...
<head>
	...
	<script type="text/javascript">
		...
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				async:{enable:(type==3),url:"${ctx}/sys/user/treeAllData?udata=${udata}",autoParam:["id=officeId"]},
					...
					onClick:function(event, treeId, treeNode){
                        ...
                        if("${checked}" == "true"){
                            if(!treeNode.isParent){
                                ...
                                if(checked){
                                    try{tree.checkNode(treeNode, false, false);}catch(e){}
                                    tree.selectNode(treeNode, true);
                                }else{
                                    try{tree.checkNode(treeNode, true, true);}catch(e){}
...
<body>
	<div style="padding: 10px"><small>关键字：</small><input type="text" id="key" style="height: 18px;font-size: 85%;" /></div>
	<div id="tree" class="ztree" style="padding: 0px 20 px 15px 20px;"></div>
</body>