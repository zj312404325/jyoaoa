...
<head>
	...
	<script type="text/javascript">
		...
		function openDialog1(url,type,description){
			type=window.encodeURI(window.encodeURI(type));
			description=window.encodeURI(window.encodeURI(description));
			url+='&type='+type;
			if(description!=null&&description!=''){
				url+='&description='+description;
			}
			openDialog('添加键值', url,'800px', '500px');
		}
	</script>
</head>
<body class="gray-bg">
	...
	<table id="contentTable" class="table table-striped table-bordered  table-hover table-condensed  dataTables-example dataTable no-footer">
		...
		<c:forEach items="${page.list}" var="dict">
			<tr>
				...
				<td>${dict.value}</td>
				<td><%-- <a  href="#" onclick="openDialogView('查看字典', '${ctx}/sys/dict/form?id=${dict.id}','800px', '500px')">${dict.label}</a> --%>${dict.label}</td>
				...
				<td>
					...
					<shiro:hasPermission name="sys:dict:add">
						<a href="#" onclick="openDialog1('<c:url value='${fns:getAdminPath()}/sys/dict/form?sort=${dict.sort+10}'></c:url>','${dict.type}','${dict.description}')" class="btn btn-primary btn-xs" ><i class="fa fa-plus"></i> 添加键值</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
