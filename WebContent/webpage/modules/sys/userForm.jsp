                ...
                $.post("${ctx}/sys/office/getPostsByOfficeId",
                    {
                        ...
                        $("#stationType").html(htmlStr);
                        $("#stationType").val('${user.stationType}');
                    });
            }
		   ...
		   <tbody>
		      <tr>
		         ...
		         <td class="width-35"><%-- <form:hidden id="nameImage" path="photo" htmlEscape="false" maxlength="255" class="input-xlarge"/>
						<sys:ckfinderhead input="nameImage" type="images" uploadPath="/photo" selectMultiple="false" maxWidth="100" maxHeight="100"/> --%></td>
		      </tr>
		      
		      <tr>
		         <td class="active"><label class="pull-right"><font color="red">*</font>归属部门:</label></td>
		         <td><%--<sys:treeselectCallBackFun id="office" name="office.id" value="${user.office.id}" labelName="office.name" labelValue="${user.office.name}"
					title="部门" url="/sys/office/treeData?type=2" cssClass="form-control required" notAllowSelectParent="false"/>--%>
					 <sys:treeselectCallBackFun id="office" name="office.id" value="${user.office.id}" labelName="office.name" labelValue="${user.office.name}"
					title="部门" url="/sys/office/treeData?type=2"  cssClass="form-control required" allowClear="${office.currentUser.admin}" />
				 </td>
		      ...
		      <tr>
		       ...
		       <tr>
		         ...
		         <td><form:input path="phone" htmlEscape="false" maxlength="100" class="form-control phone"/></td>
		      </tr>
		      
		      <tr>
		         <td class="active"><label class="pull-right">手机:</label></td>
		         <td><form:input path="mobile" htmlEscape="false" maxlength="100" class="form-control mobile"/></td>
		      ...
		      <tr>
		       ...
		       <tr>
		         ...
		         <td><%--<form:select path="stationType"  class="form-control">
					 <form:option value="" label="请选择"/>
					 &lt;%&ndash;<form:options items="${fns:getDictList('sys_station_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>&ndash;%&gt;
				 </form:select>--%>
					 <select id="stationType" name="stationType" class="form-control valid" aria-invalid="false">
