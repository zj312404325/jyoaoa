...
<body>
	...
	<table id="treeTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead><tr><th>部门</th><th>岗位</th><%--<th>职务说明书</th><th>绩效考核指标</th>--%><th>操作</th></tr></thead>
		<tbody id="treeTableList">

        <c:if test="${fn:length(office.postList)>0}">
			<c:forEach items="${office.postList}" var="post">
				<tr>
					<td>${office.name}</td>
					<td>${post.postname}</td>

					<td>
						<%--<c:if test="${post.file1 != null && post.file1 != ''}">
							<a class="frameItem btn btn-info btn-xs" href="javascript:" vl="${ctx}/sys/office/pdfView?id=${office.id}&type=1" vl2="查看-${office.name}-职务说明书" data-index="${office.id}1" >
								<i class="fa fa-search-plus"></i> 查看
							</a>
						</c:if>--%>

						<button class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" onclick="openDialog('上传职务说明书', '/a/sys/officePostDesc/form2?officeid=${office.id}&postid=${post.id}','500px', '420px')" title="档案管理添加"><i class="fa fa-plus"></i> 上传职务说明书</button>

						<c:if test="${post.officePostDesc != null && post.officePostDesc.file1 != null}">
							<a class="frameItem btn btn-info btn-sm" href="javascript:" vl="${ctx}/sys/office/viewPdf?fileurl=${post.officePostDesc.file1}" vl2="查看-${office.name}-${post.postname}-职务说明书" data-index="${post.officePostDesc.file1}" >
								<i class="fa fa-search-plus"></i> 查看职务说明书
							</a>
						</c:if>
							<c:if test="${post.officePostDesc != null && post.officePostDesc.file2 != null}">
								<a class="frameItem btn btn-info btn-sm" href="javascript:" vl="${ctx}/sys/office/viewPdf?fileurl=${post.officePostDesc.file2}" vl2="查看-${office.name}-${post.postname}-绩效考核指标" data-index="${post.officePostDesc.file2}" >
									<i class="fa fa-search-plus"></i> 查看绩效考核指标
								</a>
							</c:if>
					</td>
				</tr>
		...
		<%--<tr>
			...
			<td>${office.name}</td>
			<td>
				<c:if test="${office.name != null && office.file1 != null}">
                    <a class="frameItem btn btn-info btn-xs" href="javascript:" vl="${ctx}/sys/office/pdfView?id=${office.id}&type=1" vl2="查看-${office.name}-职务说明书" data-index="${office.id}1" >
                        <i class="fa fa-search-plus"></i> 查看
                    </a>
				</c:if>
		...
		<tr>
			...
			<td>${office.name}</td>
			<td>
				<c:if test="${office.name != null && office.file2 != null}">
					<a class="frameItem btn btn-info btn-xs" href="javascript:" vl="${ctx}/sys/office/pdfView?id=${office.id}&type=2" vl2="查看-${office.name}-绩效考核指标" data-index="${office.id}2" >
						<i class="fa fa-search-plus"></i> 查看
					</a>
				</c:if>
			</td>
