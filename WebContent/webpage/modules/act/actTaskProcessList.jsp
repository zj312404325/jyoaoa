		...
		<tbody>
			<c:forEach items="${page.list}" var="object">
				...
				<tr>
					...
					<%-- <td><a href="${ctx}/act/task/form?procDefId=${process.id}&processkey=${process.key}">${process.key}</a></td> --%>
					<td>${process.name}</td>
					<td><a  href="javascript:openDialogView('流程图','${ctx}/act/process/resource/read?procDefId=${process.id}&resType=image','1000px', '600px')">${process.diagramResourceName}</a>	</td>
					<%-- <td><b title='流程版本号'>V: ${process.version}</b></td> --%>
					<%-- <td><fmt:formatDate value="${deployment.deploymentTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%>
					<td>
						<a href="${ctx}/act/task/form?procDefId=${process.id}&processkey=${process.key}">启动流程</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
