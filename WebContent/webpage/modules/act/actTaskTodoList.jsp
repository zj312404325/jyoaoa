				...
				<tr>
					<td>
						<c:if test="${empty task.assignee}">
							<a href="javascript:claim('${task.id}','${task.processInstanceId}');" title="签收任务">${fns:abbr(not empty act.vars.map.title ? act.vars.map.title : task.id, 60)}</a>
						</c:if>
					...
					<td>
						<%-- <a  href="javascript:openDialogView('跟踪','${ctx}/act/task/trace/photo/${task.processDefinitionId}/${task.executionId}','1000px', '600px')">${task.name}</a> --%>
						${task.name}
					</td>
					...
					<td>
						<c:if test="${empty task.assignee}">
							<a href="javascript:claim('${task.id}','${task.processInstanceId}');">签收任务</a>
						</c:if>
						<c:if test="${not empty task.assignee}"><%--
							<a href="${ctx}${procExecUrl}/exec/${task.taskDefinitionKey}?procInsId=${task.processInstanceId}&act.taskId=${task.id}">办理</a> --%>
							<c:if test="${act.vars.map.applyUserId==fns:getUser().loginName&&act.vars.map.adjust!=null}">
							<%-- <c:if test="${task.name=='调整申请'}"> --%>
							<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&adjust=1">调整申请</a>
							</c:if>
							<c:if test="${act.vars.map.applyUserId!=fns:getUser().loginName||act.vars.map.adjust==null}">
							<%-- <c:if test="${task.name!='调整申请'}"> --%>
						...
						<a  href="javascript:openDialogView('跟踪','${ctx}/act/task/trace/photo/${task.processDefinitionId}/${task.executionId}','1000px', '600px')">跟踪</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
