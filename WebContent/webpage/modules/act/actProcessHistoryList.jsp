...
<body class="gray-bg">
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<tbody>
			<c:forEach items="${list}" var="procIns" varStatus="n">
				<tr>
					<td>${procIns.startUserId}</td>
					<td>${procIns.id}</td>
					<td>${page.list[n.count-1].processInstanceId}</td>
					<td>${procIns.processDefinitionId}</td>
					<td><%-- <fmt:formatDate value="${procIns.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/> --%>${procIns.startTime}</td>
					<td><%-- <fmt:formatDate value="${procIns.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/> --%>${procIns.endTime}</td>
