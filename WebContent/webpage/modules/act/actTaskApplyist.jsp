...
<head>
	...
	function page(n,s){
        	location = '${ctx}/act/task/apply/?pageNo='+n+'&pageSize='+s;
        }
	</script>
</head>
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="act" action="${ctx}/act/task/apply/" method="get" class="form-inline">
		...
		<div class="form-group">
			...
			<select id="applystatetype" name="applystatetype" class="form-control">
				<option value="" <c:if test="${applystatetype==null}">selected</c:if> >全部</option>
				<option value="0" <c:if test="${applystatetype=='0'}">selected</c:if> >运行中</option>
				<option value="1" <c:if test="${applystatetype=='1'}">selected</c:if> >已撤销</option>
				<option value="2" <c:if test="${applystatetype=='2'}">selected</c:if> >已完成</option>
			</select>
			<label>时间：</label>
		...
		<tbody>
			<c:forEach items="${page.list}" var="map">
				...
				<tr>
					...
					<td>${map.APPLYUSERID}</td>
					<td>${map.STARTTIME}</td>
					<td>
						<c:choose>
						<c:when test="${map.DELETEREASON==null&&map.ENDTIME==null}">运行中</c:when>
						<c:when test="${map.DELETEREASON!=null&&map.ENDTIME!=null}">已撤销</c:when>
						<c:otherwise>
							已完成
						</c:otherwise>
						</c:choose>
					...
					<td>
						<a href="${ctx}/act/task/form?taskId=${map.TASKID}&taskName=${fns:urlEncode(map.TASKNAME)}&taskDefKey=${map.TASKDEFKEY}&procInsId=${map.PROCESSINSTANCEID}&procDefId=${map.PROCESSDEFINITIONID}&flag=1">详情</a>
						&nbsp;<a  href="javascript:openDialogView('跟踪','${ctx}/act/task/trace/photo/${map.PROCESSDEFINITIONID}/${map.EXECUTIONID}','1000px', '600px')">跟踪</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
