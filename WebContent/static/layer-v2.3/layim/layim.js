...
layui.use('layim', function(layim){
	...
	// 接收到消息的回调方法
	  webSocket.onmessage = function(event) {
	    var res=event.data;
	    if(res.indexOf("_remind_window_")>=0){
	    	setTimeout(function(){ (showMsgCount(res.replace("_remind_window_","")));},5000);
	    }

          if(res.indexOf("_online_all_systemnotification_")>=0){
              var dd = res.replace("_online_all_systemnotification_","")
              setTimeout(function(){updateSystemnotification(dd);},500);

          }

		if(res.indexOf("_online_all_status_on_")>=0){
			var dd = res.substring("_online_all_status_on_".length);
			setTimeout(function(){updateOnlineStatus(eval('('+dd+')').body.data,eval('('+dd+')').body.userdata);},500);
			
		}
		if(res.indexOf("_online_all_status_hide_")>=0){
              var dd = res.substring("_online_all_status_hide_".length);
              setTimeout(function(){updateOnlineStatus(eval('('+dd+')').body.data,eval('('+dd+')').body.userdata);},500);
        }
        if(res.indexOf("_online_all_status_leave_")>=0){
              var dd = res.substring("_online_all_status_leave_".length);
              setTimeout(function(){updateOnlineStatus(eval('('+dd+')').body.data,eval('('+dd+')').body.userdata);},500);

        }
		if(res.indexOf("_online_all_status_off_")>=0){
              var dd = res.substring("_online_all_status_off_".length);
              setTimeout(function(){updateOnlineStatus(eval('('+dd+')').body.data,eval('('+dd+')').body.userdata);},500);
    ...
    //切换离线状态的方法
      function setoffline() {
        webSocket.send("_offline_user_"+currentId);
      };
    //切换离开状态的方法
      function setleaveout() {
        webSocket.send("_leaveout_user_"+currentId);
      };
	//切换隐身状态的方法
	  function sethide() {
    ...
    function updateSystemnotification(msg)//更新系统通知
    {
        $("#systemNotificationArea").html(msg);
    }

	//更新在线用户信息
	  function updateOnlineStatus(arra,userdata)//更新在线用户信息
	  {  var onlineNum = 0;
	  	 $("div.layui-layim-main ul.layim-list-friend li ul.layui-layim-list li").each(function(){//状态还原
	  		 if($(this).attr("id")){
		  		 ...
		  		 if((','+arra+",").indexOf(','+name+',')>=0){
					 ...
					 }else if(sts == '_leaveout_user_'){
                         span.html(text +"(<font color='orange'>离开</font>)");
                          var $one_li = $(this).parent().find("li:eq("+onlineNum+")");    //  获取<ul>节点中第一个<li>元素节点
                         $(this).insertBefore($one_li);    //移动节点
                     }else if(sts == '_leave_user_'){
                         span.html(text +"(<font color='color'>离线</font>)");
					 }else if(sts == '_offline_user_'){
                         span.html(text +"(<font color='color'>离线</font>)");
                     }else{
                         span.html(text +"(<font color='green'>在线</font>)");
                         var $one_li = $(this).parent().find("li:eq(0)");    //  获取<ul>节点中第一个<li>元素节点
