...
layui.define(['jquery', 'layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ,'<div class="layui-layim-info">'
      ...
      ,'<div class="layui-layim-status">'
        ...
        ,'<ul class="layui-anim layim-menu-box">'
          ...
          ,'<li {{d.mine.status === "offline" ? "class=layim-this" : ""}} layim-event="status" lay-type="offline"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-offline">&#xe617;</cite>下线</li>'
        ,'</ul>'
      ,'</div>'
    ...
    ,'<ul class="layui-layim-tool">'
      ...
      ,'<li class="glyphicon glyphicon-trash" style="font-size: 15px; font-weight:normal" layim-event="gc" title="清理聊天记录"></li>'
      /*,'{{# if(d.base.find){ }}'
  ...
  layui.laytpl.date = function(timestamp){
    ...
    .replace(/\//g, "-").replace(/(\s)\D*(\d)/g, "$1$2");*/
	  return new Date(timestamp||new Date()).toLocaleString().replace(/年|月/g, "-").replace(/日/g, " ");
  };
  ...
  var anim = 'layui-anim-up', events = {
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"id="layim-'+ data[l].type + data[l].id +'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
    ...
    ,skin: function(){
      ...
      ,gc: function(){
          layer.open({
              content:'您确认要删除即时聊天记录吗？',
              btn: ['删除缓存记录','删除聊天记录','取消'], //按钮
              ...
              btn2:function (index, layero) {
                  //删除聊天记录
                  var iload = layer.load(0, {shade: false});
                  $.post("/a/iim/chatHistory/deleteMyChatHistory",
                      {
                          "type":"friend"
                      },function(data){
                          layer.close(iload);
