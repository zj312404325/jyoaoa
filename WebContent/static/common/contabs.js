...
$(function () {
	...
	$('.J_menuItem2').on('click', menuItem2);
...
function menuItem2() {
	...
	var menuName = $.trim($(this).children().not(".text-danger").text());
	var flag = true;
...
function menuItem() {
    ...
    var dataUrl = $(this).attr('href'),
        ...
        menuName = $.trim($(this).children().not(".text-danger").text()),
        flag = true;
    $(this).closest('.dropdown').removeClass('open').find('a.dropdown-toggle').attr('aria-expanded',false);
...
//关闭指定选项卡菜单
function closeTheTab(closeTabId) {
	//var closeTabId = $(this).parents('.J_menuTab').data('id');
	//var currentWidth = $('.J_iframe[data-id="' + closeTabId + '"]').width();
	window.parent.$('.page-tabs-content').children("[data-id]").not(":first").each(function () {
		if(closeTabId == $(this).data('id')){
			$('.J_iframe[data-id="' + $(this).data('id') + '"]').remove();
	        $(this).remove();
		}
    });
...
function addFrameItem(dataUrl,dataIndex,menuName){
	if (dataUrl == undefined || $.trim(dataUrl).length == 0)return false;
	var flag = true;
	// 选项卡菜单已存在
    $('.J_menuTab',parent.document).each(function () {
