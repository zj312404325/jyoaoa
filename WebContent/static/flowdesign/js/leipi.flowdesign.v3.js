   ...
   $.fn.Flowdesign = function(options)
    {
        ...
        if( /msie/.test(navigator.userAgent.toLowerCase()) && $.support.leadingWhitespace ){ //ie9以下，用VML画图
    ...
    if(processData.list)
    {
        $.each(processData.list, function(i,row) 
        {
            ...
            var nodeId = "window" + row.id, badge = 'badge-inverse',icon = 'glyphicon-star';
            ...
            .html('<span class="process-flag badge '+badge+'"><i class="glyphicon '+icon+' icon-white"></i></span>&nbsp;' + row.process_name )
