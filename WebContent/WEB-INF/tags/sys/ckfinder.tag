...
<%@ attribute name="viewonly" type="java.lang.Boolean" required="false" description="是否查看模式"%>
...
<div style="position:relative"><ol id="${input}Preview"></ol><c:if test="${!viewonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn btn-primary">${selectMultiple?'添加':'选择'}</a></c:if><c:if test="${readonly}">&nbsp;<a href="javascript:" id="savefiles" class="btn btn-success">提交</a></c:if><small style="position:absolute; right:0; bottom:0; color:#ccc;">文件不得超过5M，图片不得超过2M。</small></div>
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		for(var i=0; i<files.length; i++){//<c:if test="${type eq 'thumb'}">
			url += files[i].getThumbnailUrl()+','+'${fns:getUser().id}';//</c:if><c:if test="${type ne 'thumb'}">
			url += files[i].getUrl()+','+'${fns:getUser().id}';//</c:if>
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li><img src=\""+urls[i].split(",")[0]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
				li = "<li><a vl=\""+urls[i].split(",")[0]+"\" url=\""+urls[i]+"\" target=\"_blank\" onclick=\"fileDownLoads(this)\">"+decodeURIComponent(urls[i].split(",")[0].substring(urls[i].split(",")[0].lastIndexOf("/")+1))+"</a>";//</c:if>
				if(urls[i].split(",")[1] == "${fns:getUser().id}"){
